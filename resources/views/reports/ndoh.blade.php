<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h2 class="heading-page">NDoH National Report</h2>
        </div>
    </x-slot>
    ...
    <x-page-container>
        ...
        <div class="surface-panel p-6">
            <form method="GET" action="{{ getDashboardNdohReportRoute('index') }}" class="mb-6 flex flex-wrap items-end gap-4">
                <div>
                ...
                <a href="{{ getDashboardNdohReportRoute('print', ['date_from' => $from->toDateString(), 'date_to' => $to->toDateString()]) }}"
                ...
                <a href="{{ getDashboardNdohReportRoute('export', ['date_from' => $from->toDateString(), 'date_to' => $to->toDateString()]) }}"
                   class="rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-800">
                    Download CSV
                </a>
            </form>

            @include('reports.partials.ndoh-summary', ['report' => $report])
        </div>
    </x-page-container>
</x-app-layout>
