...
class NdohReportService
{
    ...
    public static function generate(?Carbon $from = null, ?Carbon $to = null): array
    {
        ...
        return [
            ...
            'shipments' => [
                // Operational totals exclude cancelled; cancelled kept as its own count.
                'total' => $shipments->where('status', '!=', 'cancelled')->count(),
    ...
    public static function toCsvRows(array $report): array
    {
        return [
            ...
            ['', ''],
            ['Metric', 'Value'],
            ['NDoH batches (on hand now)', $report['inventory']['total_batches']],
            ['NDoH units on hand (now)', $report['inventory']['total_units']],
