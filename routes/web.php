...
use App\Http\Controllers\DrugIssuanceController;
use App\Http\Controllers\DrugReturnController;
use App\Http\Controllers\DrugUsageController;
...
use App\Http\Controllers\WardStockController;
...
Route::middleware(['auth', 'verified', 'role:pharmacy_manager'])->prefix('pharmacy-manager')->name('pharmacy-manager.dashboard.')->group(function () {
    ...
    Route::resource('drug-issuances', DrugIssuanceController::class)->except(['edit', 'update']);
    Route::post('drug-issuances/{issuance}/receive', [DrugIssuanceController::class, 'receive'])->name('drug-issuances.receive');
    Route::resource('drug-returns', DrugReturnController::class)->except(['edit', 'update']);
    Route::post('drug-returns/{return}/receive', [DrugReturnController::class, 'receive'])->name('drug-returns.receive');
    Route::resource('drug-usages', DrugUsageController::class)->only(['index', 'create', 'store', 'show', 'destroy']);
    Route::resource('ward-stocks', WardStockController::class)->only(['index']);
    Route::get('ward-stocks/{ward}', [WardStockController::class, 'show'])->name('ward-stocks.show');
    Route::patch('ward-stocks/{wardStock}/reorder-point', [WardStockController::class, 'updateReorderPoint'])->name('ward-stocks.update-reorder-point');
    Route::get('reports/stock-status', [StockStatusController::class, 'index'])->name('reports.stock-status.index');
    Route::get('reports/stock-movements', [StockMovementController::class, 'index'])->name('reports.stock-movements.index');
